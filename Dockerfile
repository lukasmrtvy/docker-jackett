...
RUN adduser -D -u ${UID} jackett && \
    apk add --no-cache --update curl openssl libcurl tar bzip2 mono supervisor --update-cache --repository http://alpine.gliderlabs.com/alpine/edge/testing/ --allow-untrusted  && \
